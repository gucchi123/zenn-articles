...
自動投稿・自動生成のパイプラインを個人プロジェクトで20本近く動かしていて、同じ形の事故を**5回**やりました。
...
5. 関門は生きていた → **同期の競合でファイルごと2週間前の版に巻き戻っていた**
...
## 5回目: 関門そのものではなく、関門の**供給元**が入れ替わっていた

これは別の日に踏んだもので、いちばん見つけにくい形でした。

判定ルールは1ファイルに集約してあります。ある日、その中の関数を呼んだら `AttributeError` になりました。**同じ日の午前中は動いていた関数**です。

見に行くと、ファイルが2週間前の版に戻っていました。原因はクラウド同期の競合解決です。別端末との差分を検知して、**新しい方を別名に退避し、古い方を正として置いていました**。

```
rules.py                16,213B  Jul 29 17:35  ← 正として使われていた（古い）
rules-<PC名>.py         20,960B  Aug  9 03:25  ← 本来の最新（退避されていた）
```

消えていたのは、後から足した検査3つです。関数が消えたものは `AttributeError` で気づけました。
ただ、**正規表現などデータだけの劣化は例外を出しません。** 判定が静かに緩くなるだけです。

対処は2つに分かれます。

- **共有の判定モジュールはバージョン管理に入れる。** 同期フォルダだけに置くと巻き戻りが検出できない
- 使う前に、**その関門が生きているかを1回確かめる**

```python
for name in ("fabrication_reason", "foreign_glyph_reason"):
    assert hasattr(rules, name), f"{name} が消えている（供給元が巻き戻った疑い）"
```

ここまでの4回は「関門を通っていない」話でしたが、5回目は**関門の中身が別物に差し替わっていた**話です。
配線を検査していても、配線先が入れ替われば意味がありません。
...
5回とも、原因は「共通化が足りない」ではなく「共通化した**つもり**の範囲が実際より狭かった」ことでした。
