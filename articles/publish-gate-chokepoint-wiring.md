...
## 4回目: 関門が「どこからも呼ばれていなかった」

ここまでで打ち止めのつもりでしたが、もう1回踏みました。今度はいちばん単純な形です。

コメント先の候補を選ぶ工程に、「同じ紹介プログラムを回している相手を除外する」関門を置いてありました。ある朝、候補の上位5件を人の目で確認したら、**5件とも除外対象**でした。1件も落ちていません。

「除外条件が甘いのだろう」と考えてコードを開き、そこで気づきました。

```python
def is_referral_blog(url):
    """本文に宣伝マーカーがあれば除外対象とみなす。"""
    ...
```

関数はありました。中身も正しく、単体テストも通ります。**呼び出しが1行もありませんでした。**

しかも候補を供給する経路は2つあり、片方は**別の**除外ヘルパを呼び、もう片方は何も呼んでいませんでした。だから完全な無風ではなく「たまに何か落ちている」状態に見えていて、それが発見を遅らせました。

ここが2回目（配線漏れ）との違いです。2回目は「一部の経路が関門を通っていなかった」でしたが、今回は**関門そのものがどの経路からも呼ばれていなかった**。それでもテストは緑です。

> **関数のテストは、その関数が呼ばれていないことを絶対に検出できない。**

対処は、この記事で書いてきた「経路を機械的に洗う」の延長です。ロジックではなく**呼び出しの存在**を検査します。

```python
src  = io.open("scout.py", encoding="utf-8").read()
main = src[src.index("def main"):]

ck("関門が実際に呼ばれている", "is_referral_blog(" in main)
ck("経路Aも同業チェックを通る", "looks_like_competitor" in body_of("scout_hatena", "main"))
```

行儀は良くありません。リファクタで関数名を変えれば落ちます。ただ、**落ちてほしいのはまさにそのとき**です。半年後に構造を変える自分は、その関門が何を守っていたか覚えていません。

### おまけ: 直したあとも半分素通りしていた

呼び出しを足して再実行したら、除外できたのは5件中1件だけでした。判定には相手の本文を取りに行く必要があり、その取得が半分以上失敗していたのです。
...
呼び出し側は `if hits:` です。**`None` は偽なので、「判定できなかった」が「問題なかった」として扱われていました。**

通信が不安定な日ほどたくさん通します。いちばん危ない日にいちばん効かなくなる作りでした。真偽の2値をやめ、**判定不能を第3の状態**として持たせています。

```python
if hits:                     # クロ
    dropped.append(blog_id)
    continue
if hits is None:             # 🔴 不明をシロにしない
    mark = "⚠"
    title = "[本文未確認] " + title
```

`if hits:` と `if hits is None:` を別々に書く。それだけですが、書いていなければ永遠に起きません。
...
- **関門が呼ばれていること自体をテストする。** 関数のテストでは「呼ばれていない」を検出できない
- **「判定できなかった」を「問題なし」に畳まない。** 取得が失敗した瞬間に全部通ることになる
- **関門が呼ばれていること自体をテストする。** 関数のテストでは「呼ばれていない」を検出できない
- **判定できなかった、を「問題なし」に畳まない。** 取得が失敗した瞬間に全部通ることになる

4回とも、原因は「共通化が足りない」ではなく「共通化した**つもり**の範囲が実際より狭かった」ことでした。
4回目にいたっては、その範囲が**ゼロ**だったわけです。
