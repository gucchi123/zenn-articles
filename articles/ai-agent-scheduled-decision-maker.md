...
```mermaid
flowchart TD
  S["スケジューラ: 定期起動<br/>(cron / Task Scheduler)"] --> A["AIエージェント起動<br/>(CLI headless + 判断プロンプト)"]
  A --> R["現状を読む<br/>直近の作業ログ / 既存成果物 / ルール"]
  R --> J{"他人の学びになる<br/>価値ある題材はあるか?"}
  J -->|なし| SKIP["SKIP<br/>(無理に作らない)"]
  J -->|あり| DO["1件だけ実行<br/>生成 → 検証 → 記録"]
  SKIP --> N["結果を通知<br/>WROTE / SKIPPED"]
  DO --> N
```
